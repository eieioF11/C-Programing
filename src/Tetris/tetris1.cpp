...
void InputKey(int *cmd,int *x,int *y,int *angle,bool xlstop,bool xrstop,bool ystop){
	...
	if (kbhit())
  {
    ...
    *cmd=1;
    ...
    {
      //case 'w':*y-=1;break;
      case 's':if(!ystop)*y+=1;break;
      case 'a':if(!xlstop)*x-=1;break;
      case 'd':if(!xrstop)*x+=1;break;
      case 'z':
        if(*angle<=0)
          *angle=3;
        else
...
void VariableClear()
{
  for(int i=0;i<fy;i++)
  {
    for(int j=0;j<fx;j++)
    {
      if(brockloceation[i-1][j-1]==0)field[i][j]=BLACK;
      //else field[i][j]=WHITE;
      if(i==0)field[i][j]=WHITE;
      if(j==0)field[i][j]=WHITE;
      if(i==fy-1)field[i][j]=WHITE;
      if(j==fx-1)field[i][j]=WHITE;
    }
  }
}
  ...
  while(true)
  {
    ...
    for(int i=0;i<4;i++)
    {
      for(int j=0;j<4;j++)
      {
        ...
        {
          if(end)brockloceation[y+i-1][x+j-1]=1;
          else
          {
            if(brockloceation[y+i-1][x+j]==1)xrstop=true;
            //else xrstop=false;
            if(brockloceation[y+i-1][x+j-2]==1)xlstop=true;
            //else xlstop=false;
          ...
          if(i!=-1||j!=-1)field[y+i][x+j]=color;
        }
      }
    }
    if(end)
